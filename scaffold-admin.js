...
const adminPages = [
  'forgot-password', 'reset-password', 'products', 'categories', 
  'orders', 'customers', 'custom-orders', 'testimonials', 
  'gallery', 'media', 'newsletter', 'settings/website', 'settings/seo',
  'settings/social', 'settings/email', 'settings/whatsapp', 'roles', 'activity-logs'
];
...
adminPages.forEach(page => {
  ...
  let title = page.split('/').pop().split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
...
export default function ${title.replace(/\s+/g, '')}Page() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-serif text-primary mb-2">${title} Management</h1>
          <p className="text-foreground/60 text-sm">Manage and organize your ${title.toLowerCase()} from here.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-secondary/10">
              <tr className="hover:bg-secondary/5 transition-colors">
                ...
                <td className="p-6 text-sm font-medium text-primary">Sample ${title}</td>
  ...
  // Simplify forgot/reset password pages
